...
impl Storage {
    ...
    pub fn get_object_by_id(&self, id: &str) -> Result<Option<KnowledgeObject>> {
        let conn = self.get_connection()?;
        ...
        let obj = stmt.query_row(params![id], Self::row_to_object).optional()?;
        Ok(obj)
    }
    ...
    pub fn search_fts(&self, query: &str, limit: usize) -> Result<Vec<KnowledgeObject>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT ko.id, ko.object_type, ko.title, ko.summary, ko.content,
             ...
             WHERE ko.original_id LIKE ?1
                OR ko.id IN (SELECT id FROM knowledge_fts WHERE knowledge_fts MATCH ?2)
             ORDER BY ko.updated_at DESC
             LIMIT ?3",
        )?;

        let rows = stmt.query_map(
            params![like_query, formatted_query, limit as i64],
            Self::row_to_object,
        )?;
