        ...
        "tools/call" => {
            ...
            match name {
                "atx_search" | "atlas_search" | "atlas_query" => {
                    ...
                    // Offload synchronous SQLite calls to tokio spawn_blocking
...
mod tests {
    ...
    #[tokio::test]
    async fn test_mcp_protocol_handlers() -> Result<()> {
        let temp_file = NamedTempFile::new()?;
        let storage = Storage::new(temp_file.path())?;

        // Test initialize
        let init_resp = handle_request(&storage, "initialize", None).await?;
        assert_eq!(init_resp["serverInfo"]["name"], "atlas-mcp");

        // Test tools/list
        let tools_resp = handle_request(&storage, "tools/list", None).await?;
        let tools = tools_resp["tools"].as_array().unwrap();
        assert_eq!(tools.len(), 2);
        assert_eq!(tools[0]["name"], "atx_search");

        // Test tools/call atx_status
        ...
        .await?;
        let text = status_resp["content"][0]["text"].as_str().unwrap();
        assert!(text.contains("Total Knowledge Objects"));
