...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Mcp => {
            ...
            mcp::run_stdio_mcp_server(storage).await?;
        }
    }
...
fn print_cli_results(objects: &[domain::KnowledgeObject]) {
    if objects.is_empty() {
