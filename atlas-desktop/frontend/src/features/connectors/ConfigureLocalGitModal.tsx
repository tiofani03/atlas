...
import { X, Loader2, Plus, Trash2, HardDrive, FolderOpen, Check, AlertCircle } from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto flex-1">
            ...
            <div className="p-3 bg-slate-50/80 dark:bg-zinc-950/60 border border-slate-200/80 dark:border-zinc-800/80 rounded-xl space-y-1 text-[11px] text-slate-500 dark:text-zinc-400">
              ...
              {paths.some((p) => p.startsWith('./') || p.startsWith('../')) && (
                <p className="text-amber-600 dark:text-amber-400 font-medium pt-1">
                  ⚠️ Note: Relative paths (e.g. <code className="font-mono">./...</code>) are resolved relative to the Atlas backend server directory. Use absolute paths (e.g. <code className="font-mono">/home/...</code> or <code className="font-mono">~/...</code>) if repositories are stored elsewhere.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-zinc-800/80">
            <button
              ...
              disabled={isValidating || paths.every((p) => !p.trim())}
              className="px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 text-slate-700 dark:text-zinc-300 text-xs font-semibold transition disabled:opacity-50 flex items-center gap-1.5 shadow-2xs active:scale-95"
            >
