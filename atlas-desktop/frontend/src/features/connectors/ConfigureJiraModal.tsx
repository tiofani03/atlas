...
export const ConfigureJiraModal: React.FC<ModalProps> = ({ isOpen, onClose, onSuccess, initialConfig }) => {
  const [id, setId] = useState(initialConfig?.id || 'jira-main');
  ...
  const [projects, setProjects] = useState(initialConfig?.projects?.join(', ') || 'PAY,DEV');
